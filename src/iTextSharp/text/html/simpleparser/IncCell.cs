    ...
    */
    public class IncCell : ITextElementArray
    {
        ...
        public IncCell(String tag, ChainedProperties props)
        {
            ...
            //Check if any inline style is included
            string style = props.ConcatPropertyUntilTag("style",HtmlTags.TABLE);
            float styleWidth = 0;
            if (!string.IsNullOrEmpty(style))
            {
                InlineStyle inStyle = new InlineStyle();
                inStyle.StyleCell(style, cell, out styleWidth);
                Width = styleWidth;
            }

            string value = props.GetCurrentProp("colspan");
            if (value != null)
                cell.Colspan = int.Parse(value);
            value = props.GetCurrentProp("rowspan");
            if (value!=null)
            {
            ...
            value = props["width"];
            if (!string.IsNullOrEmpty(value) && styleWidth == 0)
            {
                float num;

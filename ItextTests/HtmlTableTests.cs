...
namespace ItextTests
{
    ...
    public class HtmlTableTests
    {
        ...
        {
            Document doc = new Document();
            PdfWriter objWriter = PdfWriter.GetInstance(doc, new FileStream("Parse_Tables_CellWidthSet_Colspan.pdf", FileMode.Create));
            ...
            string strTexto = @"<p>teste bruto</p><table border=""1""><tbody><tr><td width=""50%"" colspan=""2"">teste</td><td width=""50%"">sdfsfsfsdf</td></tr><tr><td>col 1</td><td>col 2</td><td>col 3</td></tr></tbody></table>";
            var objects = HTMLWorker.ParseToList(new StringReader(strTexto), styles);
            foreach (var element in objects)
            {
                var objParagraph = new Paragraph();
